...
def tokenize(string):
    ...
    """
    return [x for x in re.sub(r'\W+',' ',string).lower().split() if x not in stopwords]
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return sorted(vendorRDD.collect(), key = lambda x: -len(x[1]))
...
def dotprod(a, b):
    ...
    """
    return sum([a[k]*b[k] for k in list(set(a.keys()).intersection(set(b.keys())))])
...
similarities = (crossSmall
                .map(lambda r: computeSimilarity(r))
                .cache())
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (a, g, c): (' '.join([g,a]), c))

trueDupsRDD = (sims
               .join(goldStandard))
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.map(lambda (k, (w, v)): w).reduce(lambda a, b: a+b)/float(trueDupsCount)

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)
              .filter(lambda (k,(w, v)): v is None))
avgSimNon = nonDupsRDD.map(lambda (k, (w, v)): w).reduce(lambda a, b: a+b)/float(nonDupsRDD.count())
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda (k,v): (k, norm(v))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda (k,v): (k, norm(v))).collectAsMap()
...
def invert(record):
    ...
    """
    pairs = [(t, record[0]) for t in record[1]]
    return (pairs)
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = sum([amazonWeightsBroadcast.value[amazonRec][k]*googleWeightsBroadcast.value[googleRec][k] for k in tokens])
    value = s/amazonNormsBroadcast.value[amazonRec]/googleNormsBroadcast.value[googleRec]
    key = (amazonRec, googleRec)
    return (key, value)

similaritiesFullRDD = (commonTokens
                       .map(lambda r: fastCosineSimilarity(r))
                       .cache())
