...
parsedSamplePoints = rawData.map(lambda s: parsePoint(s))
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(labelsAndPreds.map(lambda (a, b): squaredError(a, b))
                     .mean())
...
alpha = 1.0
miniBatchFrac = 1.0
for reg in [math.pow(0.1,10), math.pow(0.1,5), 1]:
...
for alpha in [math.pow(0.1, 5), 10]:
...
def twoWayInteractions(lp):
    ...
    return LabeledPoint(lp.label, np.hstack((lp.features,
                                   [a*b for (a,b) in list(itertools.product(lp.features, 
                                                                   lp.features))])))
