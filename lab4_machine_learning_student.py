...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    n = len(list(IDandRatingsTuple[1]))
    return (IDandRatingsTuple[0],(n, sum(IDandRatingsTuple[1])/float(n)))
...
movieIDsWithRatingsRDD = (ratingsRDD.map(lambda (a, b, c): (b, c))
                          .groupByKey())
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda ((a,b), (rp, ra)): math.pow((rp-ra), 2))) 
...
myUnratedMoviesRDD = (moviesRDD
                      .map(lambda (a, b): (myUserID, a))
                     .filter(lambda (a, b): b not in [789, 603, 1110, 587, 1047, 744, 546, 1248, 831, 1300]))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (a, (b,c)): (a, b))
